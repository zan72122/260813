...
| `beat-pipe-cutaway` | pipe-run | **図鑑風断面図**。専用の直線「表示用配管セグメント」を真横から見る（レールはほぼ側面固定・緩い並走のみ）。画面レイヤ構成: 上部に芝生ライン＋生垣/空の細帯（地上の文脈）、その下に明るめの暖色土壌帯、中央に切り開いた配管断面、内部を走る明るい水塊（先端が最も明るく、濡れた尾を引く）。終端で水塊が噴水の立ち上がり管へ入り、カメラも一緒に地上へ抜ける | 2.5–3.5s | 縦画面: バルブ(上)→下降→横走り→basin(下)のZ字/L字。横画面: バルブ(左)→basin(右)の横一直線。土壌を黒く潰さない（近黒禁止）。水塊の輝度コントラストが画面の主役。到達(t=1.0)まで水塊は常に可視 |
