...
export class EiffelStateMachine {
  ...
  update(dt: number, drive: DriveReadout, leveling: LevelingReadout): void {
    ...
    this.updateMotionSoundCues(drive);
    ...
    switch (this.stateId) {
      ...
      case 'ascendLower':
        ...
        break;
      case 'transition':
        if (leveling.settled && drive.arcLength >= BLEND_END_S) {
          this.fire('LEVELED');
        }
        break;
      case 'ascendUpper':
  ...
  private resetTimersForEntry(id: GameStateId): void {
    ...
    if (id === 'transition') {
      ...
      this.prevLevelingSettled = false; // a fresh transition entry always starts unsettled, so settling fires the success cue exactly once
    }
  }
  ...
  private updateMotionSoundCues(drive: DriveReadout): void {
